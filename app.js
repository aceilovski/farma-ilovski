...
function showToast(t) { let b = document.getElementById('toastNotification'); b.innerText = t; b.classList.add('show'); setTimeout(() => b.classList.remove('show'), 2500); }
function showAlert(t) { document.getElementById('alertText').innerText = t; document.getElementById('customAlert').classList.add('show'); }
function closeAlert() { document.getElementById('customAlert').classList.remove('show'); }
let confirmCb = null; function showConfirm(t, cb) { document.getElementById('confirmText').innerText = t; confirmCb = cb; document.getElementById('customConfirm').classList.add('show'); }
function closeConfirm(y) { document.getElementById('customConfirm').classList.remove('show'); if(y && confirmCb) confirmCb(); }
let promptCb = null; function showPrompt(t, cb) { document.getElementById('promptText').innerText = t; document.getElementById('promptInput').value = ""; promptCb = cb; document.getElementById('customPrompt').classList.add('show'); }
function closePrompt(y) { document.getElementById('customPrompt').classList.remove('show'); if(y && promptCb) promptCb(document.getElementById('promptInput').value); }
function toggleMenu() { document.getElementById('navButtons').classList.toggle('show'); }

function toggleTheme() { 
  let body = document.body; let icon = document.getElementById('themeIcon');
  body.classList.toggle('dark-mode'); 
  if(body.classList.contains('dark-mode')) { icon.className = "bi bi-sun-fill text-warning"; localStorage.setItem('theme', 'dark'); } 
...
function applyUserRoleUI() {
    ...
    if (currentRole === 'superadmin') { 
        rb.innerHTML = '<i class="bi bi-star-fill me-1"></i>Супер Админ'; 
        rb.className = 'badge bg-danger text-white mt-1'; 
        // Само Superadmin го гледа Админ панелот
        document.getElementById('btnAdmin').classList.remove('hidden'); 
        document.getElementById('thEdit').classList.remove('hidden'); 
    } else if (currentRole === 'admin') {
        rb.innerHTML = '<i class="bi bi-shield-lock-fill me-1"></i>Админ'; 
        rb.className = 'badge bg-warning text-dark mt-1'; 
        // Обичен админ НЕ го гледа Админ панелот
        document.getElementById('btnAdmin').classList.add('hidden'); 
        document.getElementById('thEdit').classList.remove('hidden');
    } else { 
        rb.innerHTML = '<i class="bi bi-person-fill me-1"></i>Корисник'; 
        rb.className = 'badge bg-info text-dark mt-1'; 
        document.getElementById('btnAdmin').classList.add('hidden'); 
        document.getElementById('thEdit').classList.add('hidden'); 
    }
...
function login() {
  ...
  auth.signInWithEmailAndPassword(u, p)
    ...
    .catch(err => { 
        document.getElementById('lBtn').disabled = false; 
        hideLoader(); 
        msg.innerText = "Погрешен е-маил или лозинка!"; 
    });
}
...
function otvoriNovaKravaModal() { selectedCowId = null; document.getElementById('kravaFormTitle').innerText = "Ново Грло"; document.getElementById('kravaUshen').value = ""; document.getElementById('kravaIme').value = ""; document.getElementById('kravaPol').value = "Женско"; document.getElementById('kravaRagjanje').value = ""; document.getElementById('kravaOsemenuvanje').value = ""; document.getElementById('kravaBrojOsemen').value = "0"; osveziKravaUI(); document.getElementById('modalKrava').classList.add('show'); }
function zatvoriModalKrava() { document.getElementById('modalKrava').classList.remove('show'); }
...
function renderCows() {
  ...
  originalHerd.forEach(c => {
    ...
    if(activeCowFilter === 'Стелни' && !c.inseminationDate) return;
    if(activeCowFilter === 'Отворена' && c.inseminationDate) return;

    let polIco = c.gender === 'Машко' ? '<i class="bi bi-gender-male text-primary fs-5"></i>' : '<i class="bi bi-gender-female text-danger fs-5"></i>';
    let status = "Отворена"; let statusC = "bg-success"; let osemStr = "-"; let btnEdit = `<button onclick="editCow('${c.id}')" class="btn btn-outline-primary px-2 py-1 border-0 rounded-circle me-1"><i class="bi bi-pencil"></i></button>`;
    ...
    if (c.gender === 'Машко') { status = "Бик/Теле"; statusC = "bg-dark"; } 
    else if (c.inseminationDate) {
      ...
      if(diffDays < 21) { status = "Осеменета"; statusC = "bg-primary"; } 
      else if (diffDays < 220) { status = "Стелна"; statusC = "bg-info text-dark"; alertMsg = `<div class="text-success small"><i class="bi bi-check-circle"></i> ${diffDays} ден.</div>`; } 
      else if (diffDays < 270) { status = "Засушена"; statusC = "bg-warning text-dark"; alertMsg = `<div class="text-warning text-dark small fw-bold">Засуши! (${diffDays}д.)</div>`; } 
      else { status = "Пред телење"; statusC = "bg-danger"; alertMsg = `<div class="text-danger small fw-bold">Време за телење!</div>`; btnDel = `<button onclick="otvoriTelenje('${c.id}', '${c.earTag}')" class="btn btn-sm btn-success rounded-pill px-2 py-1"><i class="bi bi-stars"></i> Отели</button>` + btnDel; alarms++; }
    }
    
    rows.push(`<tr><td><div class="mb-1">${polIco} <b class="text-dark">${c.earTag}</b> ${c.name?`(${c.name})`:''}</div><small class="text-muted">Мајка: ${c.mother||'-'}</small></td><td class="text-center"><span class="badge ${statusC} px-2 py-1">${status}</span></td><td class="text-center"><div style="font-size:0.9rem;">${osemStr}</div>${alertMsg}</td><td class="text-end" style="white-space: nowrap;">${btnEdit}${btnDel}</td></tr>`);
  });
  
  document.getElementById('tabelaKravi').innerHTML = rows.join(''); 
  document.getElementById('vkupnoKraviBroj').innerText = `Прикажани: ${rows.length}`;
  let bdg = document.getElementById('stadoBadge'); 
  if(alarms > 0) { bdg.innerText = alarms; bdg.classList.remove('hidden'); } else bdg.classList.add('hidden');
}
...
function izvestaj() {
  ...
  db.collection("milk").where("period", "==", period).get().then(snap => {
    ...
    document.getElementById('vkOsnova').innerText = osnova.toFixed(2) + " ден."; 
    document.getElementById('vkDanok').innerText = "- " + danok.toFixed(2) + " ден."; 
    document.getElementById('vkNeto').innerText = neto.toFixed(2) + " ден.";
  });
}
